...
data_split_size = None
labels_split_size = None
if rank == 0:
    data_split_size = np.array_split(data, size)
    labels_split_size = np.array_split(labels, size)

X_local = np.empty(data_split_size[0].shape, dtype=data_split_size[0].dtype) if rank == 0 else None
y_local = np.empty(labels_split_size[0].shape, dtype=labels_split_size[0].dtype) if rank == 0 else None

X_local = comm.scatter(data_split_size, root=0)
y_local = comm.scatter(labels_split_size, root=0)

print("Rank:", rank, "samples:", X_local.shape[0])
...
print("Rank:", rank, "Training size:", X_train_local.shape[0], "Test size:", X_test_local.shape[0])
